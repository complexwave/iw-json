...
static int print_channels_handler(struct nl_msg *msg, void *arg)
{
	...
	if (tb_msg[NL80211_ATTR_WIPHY_BANDS]) {
		nla_for_each_nested(nl_band, tb_msg[NL80211_ATTR_WIPHY_BANDS], rem_band) {
			...
			if (iw_json && tb_band[NL80211_BAND_ATTR_FREQS]) {
				json_obj_begin(NULL);
				...
				nla_for_each_nested(nl_freq, tb_band[NL80211_BAND_ATTR_FREQS], rem_freq) {
					...
					json_i32(JSON_OUT_JSON, "channel", NULL,
						  ieee80211_frequency_to_channel(freq));

					if (tb_freq[NL80211_FREQUENCY_ATTR_DISABLED]) {
						json_bool(JSON_OUT_JSON, "disabled", NULL, true);
						json_obj_end();
						continue;
					}
					...
					if (tb_freq[NL80211_FREQUENCY_ATTR_NO_IR] && tb_freq[__NL80211_FREQUENCY_ATTR_NO_IBSS])
						json_bool(JSON_OUT_JSON, "no_ir", NULL, true);
					else if (tb_freq[NL80211_FREQUENCY_ATTR_PASSIVE_SCAN])
						json_bool(JSON_OUT_JSON, "passive_scan", NULL, true);
					else if (tb_freq[__NL80211_FREQUENCY_ATTR_NO_IBSS])
						json_bool(JSON_OUT_JSON, "no_ibss", NULL, true);

					if (tb_freq[NL80211_FREQUENCY_ATTR_RADAR])
						json_bool(JSON_OUT_JSON, "radar", NULL, true);

					json_arr_begin("widths");
					if (!tb_freq[NL80211_FREQUENCY_ATTR_NO_20MHZ])
						json_str(JSON_OUT_JSON, NULL, NULL, "20MHz");
					if (ctx->width_40 && !tb_freq[NL80211_FREQUENCY_ATTR_NO_HT40_MINUS])
						json_str(JSON_OUT_JSON, NULL, NULL, "HT40-");
					if (ctx->width_40 && !tb_freq[NL80211_FREQUENCY_ATTR_NO_HT40_PLUS])
						json_str(JSON_OUT_JSON, NULL, NULL, "HT40+");
					if (ctx->width_80 && !tb_freq[NL80211_FREQUENCY_ATTR_NO_80MHZ])
						json_str(JSON_OUT_JSON, NULL, NULL, "VHT80");
					if (ctx->width_160 && !tb_freq[NL80211_FREQUENCY_ATTR_NO_160MHZ])
						json_str(JSON_OUT_JSON, NULL, NULL, "VHT160");
					json_arr_end();

					if (tb_freq[NL80211_FREQUENCY_ATTR_DFS_STATE]) {
						enum nl80211_dfs_state state = nla_get_u32(tb_freq[NL80211_FREQUENCY_ATTR_DFS_STATE]);

						json_str(JSON_OUT_JSON, "dfs_state", NULL, dfs_state_name(state));
						if (tb_freq[NL80211_FREQUENCY_ATTR_DFS_TIME])
							json_u32(JSON_OUT_JSON, "dfs_time_sec", NULL,
								   nla_get_u32(tb_freq[NL80211_FREQUENCY_ATTR_DFS_TIME]) / 1000);
						if (tb_freq[NL80211_FREQUENCY_ATTR_DFS_CAC_TIME])
							json_u32(JSON_OUT_JSON, "dfs_cac_time_ms", NULL,
								   nla_get_u32(tb_freq[NL80211_FREQUENCY_ATTR_DFS_CAC_TIME]));
					}

					if (tb_freq[NL80211_FREQUENCY_ATTR_CAC_START_TIME])
						json_bool(JSON_OUT_JSON, "cac_ongoing", NULL, true);
