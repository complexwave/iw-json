...
static int print_phy_handler(struct nl_msg *msg, void *arg)
{
	...
	if (iw_json) {
		...
		if (tb_msg[NL80211_ATTR_WIPHY])
			json_u32(JSON_OUT_JSON, "wiphy", NULL,
				   nla_get_u32(tb_msg[NL80211_ATTR_WIPHY]));
		if (tb_msg[NL80211_ATTR_WIPHY_NAME])
			json_str(JSON_OUT_JSON, "wiphy_name", NULL,
				     nla_get_string(tb_msg[NL80211_ATTR_WIPHY_NAME]));

		if (tb_msg[NL80211_ATTR_WIPHY_BANDS]) {
			json_arr_begin("bands");
			nla_for_each_nested(nl_band, tb_msg[NL80211_ATTR_WIPHY_BANDS], rem_band) {
				json_obj_begin(NULL);
				json_i32(JSON_OUT_JSON, "band", NULL, nl_band->nla_type + 1);

				nla_parse(tb_band, NL80211_BAND_ATTR_MAX, nla_data(nl_band),
					  nla_len(nl_band), NULL);

				if (tb_band[NL80211_BAND_ATTR_HT_CAPA])
					json_hex(JSON_OUT_JSON, "ht_capa", NULL,
						  nla_get_u16(tb_band[NL80211_BAND_ATTR_HT_CAPA]));
				if (tb_band[NL80211_BAND_ATTR_HT_AMPDU_FACTOR])
					json_u32(JSON_OUT_JSON, "ht_ampdu_factor", NULL,
						   nla_get_u8(tb_band[NL80211_BAND_ATTR_HT_AMPDU_FACTOR]));
				if (tb_band[NL80211_BAND_ATTR_HT_AMPDU_DENSITY])
					json_u32(JSON_OUT_JSON, "ht_ampdu_density", NULL,
						   nla_get_u8(tb_band[NL80211_BAND_ATTR_HT_AMPDU_DENSITY]));
				if (tb_band[NL80211_BAND_ATTR_VHT_CAPA])
					json_hex(JSON_OUT_JSON, "vht_capa", NULL,
						  nla_get_u32(tb_band[NL80211_BAND_ATTR_VHT_CAPA]));

				if (tb_band[NL80211_BAND_ATTR_FREQS]) {
					json_arr_begin("frequencies");
					nla_for_each_nested(nl_freq, tb_band[NL80211_BAND_ATTR_FREQS], rem_freq) {
						...
						json_obj_begin(NULL);
						json_u32(JSON_OUT_JSON, "freq", NULL, freq);
						if (tb_freq[NL80211_FREQUENCY_ATTR_OFFSET])
							json_u32(JSON_OUT_JSON, "freq_offset", NULL,
								   nla_get_u32(tb_freq[NL80211_FREQUENCY_ATTR_OFFSET]));
						chan = ieee80211_frequency_to_channel(freq);
						if (chan)
							json_i32(JSON_OUT_JSON, "channel", NULL, chan);
						if (tb_freq[NL80211_FREQUENCY_ATTR_MAX_TX_POWER] && !disabled)
							json_f64(JSON_OUT_JSON, "max_tx_power_dbm", NULL,
								    0.01 * nla_get_u32(tb_freq[NL80211_FREQUENCY_ATTR_MAX_TX_POWER]));

						json_bool(JSON_OUT_JSON, "disabled", NULL, disabled);
						json_bool(JSON_OUT_JSON, "no_ir", NULL,
							   !!tb_freq[NL80211_FREQUENCY_ATTR_NO_IR]);
						json_bool(JSON_OUT_JSON, "radar", NULL,
							   !!tb_freq[NL80211_FREQUENCY_ATTR_RADAR]);

						/* Same NO-IR logic as the plain-text path */
						if (tb_freq[NL80211_FREQUENCY_ATTR_NO_IR] &&
						    tb_freq[__NL80211_FREQUENCY_ATTR_NO_IBSS]) {
							/* new kernel: covered by no_ir */
						} else {
							json_bool(JSON_OUT_JSON, "passive_scan", NULL,
								   !!tb_freq[NL80211_FREQUENCY_ATTR_PASSIVE_SCAN]);
							json_bool(JSON_OUT_JSON, "no_ibss", NULL,
								   !!tb_freq[__NL80211_FREQUENCY_ATTR_NO_IBSS]);
						}
						json_obj_end();
					}
				...
				if (tb_band[NL80211_BAND_ATTR_RATES]) {
					...
					nla_for_each_nested(nl_rate, tb_band[NL80211_BAND_ATTR_RATES], rem_rate) {
						...
						if (!tb_rate[NL80211_BITRATE_ATTR_RATE])
							continue;
						json_obj_begin(NULL);
						json_f64(JSON_OUT_JSON, "bitrate_mbps", NULL,
							    0.1 * nla_get_u32(tb_rate[NL80211_BITRATE_ATTR_RATE]));
		...
		if (tb_msg[NL80211_ATTR_WIPHY_FRAG_THRESHOLD]) {
			...
			if (frag != (unsigned int)-1)
				json_u32(JSON_OUT_JSON, "frag_threshold", NULL, frag);
		}
		...
		if (tb_msg[NL80211_ATTR_CIPHER_SUITES]) {
			...
			json_arr_begin("ciphers");
			for (i = 0; i < num; i++)
				json_str(JSON_OUT_JSON, NULL, NULL, cipher_name(ciphers[i]));
			json_arr_end();
		}

		/* antenna chain bitmaps: emit as ints (consumers popcount / re-hex) */
		if (tb_msg[NL80211_ATTR_WIPHY_ANTENNA_AVAIL_TX])
			json_u32(JSON_OUT_JSON, "antenna_avail_tx", NULL,
				   nla_get_u32(tb_msg[NL80211_ATTR_WIPHY_ANTENNA_AVAIL_TX]));
		if (tb_msg[NL80211_ATTR_WIPHY_ANTENNA_AVAIL_RX])
			json_u32(JSON_OUT_JSON, "antenna_avail_rx", NULL,
				   nla_get_u32(tb_msg[NL80211_ATTR_WIPHY_ANTENNA_AVAIL_RX]));
		if (tb_msg[NL80211_ATTR_WIPHY_ANTENNA_TX])
			json_u32(JSON_OUT_JSON, "antenna_tx", NULL,
				   nla_get_u32(tb_msg[NL80211_ATTR_WIPHY_ANTENNA_TX]));
		if (tb_msg[NL80211_ATTR_WIPHY_ANTENNA_RX])
			json_u32(JSON_OUT_JSON, "antenna_rx", NULL,
				   nla_get_u32(tb_msg[NL80211_ATTR_WIPHY_ANTENNA_RX]));

		if (tb_msg[NL80211_ATTR_SUPPORTED_IFTYPES]) {
			json_arr_begin("supported_iftypes");
			nla_for_each_nested(nl_if, tb_msg[NL80211_ATTR_SUPPORTED_IFTYPES], rem_if)
				json_str(JSON_OUT_JSON, NULL, NULL, iftype_name(nla_type(nl_if)));
			json_arr_end();
		}

		if (tb_msg[NL80211_ATTR_SOFTWARE_IFTYPES]) {
			json_arr_begin("software_iftypes");
			nla_for_each_nested(nl_if, tb_msg[NL80211_ATTR_SOFTWARE_IFTYPES], rem_if)
				json_str(JSON_OUT_JSON, NULL, NULL, iftype_name(nla_type(nl_if)));
			json_arr_end();
		}

		if (tb_msg[NL80211_ATTR_SUPPORTED_COMMANDS]) {
			json_arr_begin("supported_commands");
			nla_for_each_nested(nl_cmd, tb_msg[NL80211_ATTR_SUPPORTED_COMMANDS], rem_cmd)
				json_str(JSON_OUT_JSON, NULL, NULL,
					     command_name(nla_get_u32(nl_cmd)));
			json_arr_end();
		}

		if (tb_msg[NL80211_ATTR_SUPPORT_IBSS_RSN])
			json_bool(JSON_OUT_JSON, "support_ibss_rsn", NULL, true);
			   ...
			   int argc, char **argv, enum id_input id)
{
	...
	if (iw_json) {
		for (i = 1; i <= NL80211_CMD_MAX; i++) {
			json_obj_begin(NULL);
			json_u32(JSON_OUT_JSON, "id", NULL, i);
			json_str(JSON_OUT_JSON, "name", NULL, command_name(i));
			json_obj_end();
		}
...
#define maybe_printf(...) do { if (print) printf(__VA_ARGS__); } while (0)
	...
	if (tb_msg[NL80211_ATTR_PROTOCOL_FEATURES]) {
		...
		bool split = feat & NL80211_PROTOCOL_FEATURE_SPLIT_WIPHY_DUMP;

		if (split)
			nl80211_has_split_wiphy = true;

		if (iw_json) {
			/* -q (print==false) is the internal probe from
			 * handle_info; stay silent so we don't pollute the
			 * phy output array. */
			if (print) {
				json_obj_begin(NULL);
				json_u32(JSON_OUT_JSON, "nl80211_features", NULL, feat);
				json_bool(JSON_OUT_JSON, "split_wiphy_dump", NULL, split);
				json_obj_end();
			}
			return NL_SKIP;
		}
