...
static void print_channel(struct nlattr **tb)
{
	...
	if (iw_json) {
		json_u32(JSON_OUT_JSON, "channel", NULL,
			   ieee80211_frequency_to_channel(freq));
		json_u32(JSON_OUT_JSON, "frequency", NULL, freq);
	} else {
	...
	if (tb[NL80211_ATTR_CHANNEL_WIDTH]) {
		json_str(JSON_OUT_BOTH, "channel_width",
			     ", width: %s",
			     channel_width_name(nla_get_u32(tb[NL80211_ATTR_CHANNEL_WIDTH])));
		if (tb[NL80211_ATTR_CENTER_FREQ1])
			json_u32(JSON_OUT_BOTH, "center_freq1",
				   ", center1: %d MHz",
				   nla_get_u32(tb[NL80211_ATTR_CENTER_FREQ1]));
		if (tb[NL80211_ATTR_CENTER_FREQ2])
			json_u32(JSON_OUT_BOTH, "center_freq2",
				   ", center2: %d MHz",
...
static int print_iface_handler(struct nl_msg *msg, void *arg)
{
	...
	if (iw_json) {
		...
		if (tb_msg[NL80211_ATTR_IFNAME])
			json_str(JSON_OUT_JSON, "ifname", NULL,
				     nla_get_string(tb_msg[NL80211_ATTR_IFNAME]));
		if (tb_msg[NL80211_ATTR_IFINDEX])
			json_u32(JSON_OUT_JSON, "ifindex", NULL,
				   nla_get_u32(tb_msg[NL80211_ATTR_IFINDEX]));
		if (tb_msg[NL80211_ATTR_WDEV])
			json_u64(JSON_OUT_JSON, "wdev", NULL,
				  nla_get_u64(tb_msg[NL80211_ATTR_WDEV]));
		...
		if (tb_msg[NL80211_ATTR_SSID]) {
			...
			ssid[len] = '\0';
			json_str(JSON_OUT_JSON, "ssid", NULL, ssid);
		}
		if (tb_msg[NL80211_ATTR_IFTYPE])
			json_str(JSON_OUT_JSON, "type", NULL,
		...
		if (tb_msg[NL80211_ATTR_WIPHY_TX_POWER_LEVEL]) {
			...
			json_f64(JSON_OUT_JSON, "txpower_dbm", NULL,
				    txp / 100.0);
		}
		if (tb_msg[NL80211_ATTR_TXQ_STATS]) {
			struct nlattr *txqs[NL80211_TXQ_STATS_MAX + 1];

			if (!nla_parse_nested(txqs, NL80211_TXQ_STATS_MAX,
					      tb_msg[NL80211_ATTR_TXQ_STATS], NULL)) {
				json_obj_begin("txq_stats");
				if (txqs[NL80211_TXQ_STATS_BACKLOG_BYTES])
					json_u32(JSON_OUT_JSON, "backlog_bytes", NULL,
						   nla_get_u32(txqs[NL80211_TXQ_STATS_BACKLOG_BYTES]));
				if (txqs[NL80211_TXQ_STATS_BACKLOG_PACKETS])
					json_u32(JSON_OUT_JSON, "backlog_packets", NULL,
						   nla_get_u32(txqs[NL80211_TXQ_STATS_BACKLOG_PACKETS]));
				if (txqs[NL80211_TXQ_STATS_FLOWS])
					json_u32(JSON_OUT_JSON, "flows", NULL,
						   nla_get_u32(txqs[NL80211_TXQ_STATS_FLOWS]));
				if (txqs[NL80211_TXQ_STATS_DROPS])
					json_u32(JSON_OUT_JSON, "drops", NULL,
						   nla_get_u32(txqs[NL80211_TXQ_STATS_DROPS]));
				if (txqs[NL80211_TXQ_STATS_ECN_MARKS])
					json_u32(JSON_OUT_JSON, "ecn_marks", NULL,
						   nla_get_u32(txqs[NL80211_TXQ_STATS_ECN_MARKS]));
				if (txqs[NL80211_TXQ_STATS_OVERLIMIT])
					json_u32(JSON_OUT_JSON, "overlimit", NULL,
						   nla_get_u32(txqs[NL80211_TXQ_STATS_OVERLIMIT]));
				if (txqs[NL80211_TXQ_STATS_COLLISIONS])
					json_u32(JSON_OUT_JSON, "collisions", NULL,
						   nla_get_u32(txqs[NL80211_TXQ_STATS_COLLISIONS]));
				if (txqs[NL80211_TXQ_STATS_TX_BYTES])
					json_u32(JSON_OUT_JSON, "tx_bytes", NULL,
						   nla_get_u32(txqs[NL80211_TXQ_STATS_TX_BYTES]));
				if (txqs[NL80211_TXQ_STATS_TX_PACKETS])
					json_u32(JSON_OUT_JSON, "tx_packets", NULL,
						   nla_get_u32(txqs[NL80211_TXQ_STATS_TX_PACKETS]));
				json_obj_end();
			}
		}
		if (tb_msg[NL80211_ATTR_4ADDR])
			json_bool(JSON_OUT_JSON, "4addr", NULL,
				   !!nla_get_u8(tb_msg[NL80211_ATTR_4ADDR]));
			...
			nla_for_each_nested(link, tb_msg[NL80211_ATTR_MLO_LINKS], n) {
				...
				if (tb[NL80211_ATTR_MLO_LINK_ID])
					json_u32(JSON_OUT_JSON, "link_id", NULL,
						   nla_get_u32(tb[NL80211_ATTR_MLO_LINK_ID]));
				if (tb[NL80211_ATTR_MAC]) {
					char buf[20];

					mac_addr_n2a(buf, nla_data(tb[NL80211_ATTR_MAC]));
					json_str(JSON_OUT_JSON, "addr", NULL, buf);
				}
