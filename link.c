...
static int link_bss_handler(struct nl_msg *msg, void *arg)
{
	...
	if (iw_json) {
		/*
		 * One element of the wrapper's "links" array per associated/
		 * authenticated link (multiple for an MLD). The wrapper object
		 * (dev/connected/mld/stats) is opened by handle_link; here we
		 * only emit the per-link fields and track state for the
		 * follow-up get_sta.
		 */
		...
		case NL80211_BSS_STATUS_ASSOCIATED:
			status_str = "associated";
			break;
		case NL80211_BSS_STATUS_AUTHENTICATED:
			status_str = "authenticated";
			break;
		case NL80211_BSS_STATUS_IBSS_JOINED:
			status_str = "ibss_joined";
			break;
		default:
			return NL_SKIP;
		}
		...
		json_str(JSON_OUT_JSON, "status", NULL, status_str);
		if (link_id >= 0)
			json_i32(JSON_OUT_JSON, "link_id", NULL, link_id);
		if (bss[NL80211_BSS_FREQUENCY])
			json_u32(JSON_OUT_JSON, "frequency", NULL,
				   nla_get_u32(bss[NL80211_BSS_FREQUENCY]));
		if (bss[NL80211_BSS_FREQUENCY_OFFSET])
			json_u32(JSON_OUT_JSON, "frequency_offset", NULL,
...
static int print_link_sta(struct nl_msg *msg, void *arg)
{
	...
	if (iw_json) {
		/* fields go straight into the "stats" object opened by handle_link */
		       ...
		       enum id_input id)
{
	...
	/* reset accumulated state (matters when reused via the library) */
	lr = (struct link_result){ .link_found = false };

	link_argv[0] = argv[0];

	/*
	 * JSON: a single wrapper object (the lone element of iw_exec's
	 * top-level array) with dev/connected/mld/links[]/stats{}. The
	 * per-link objects from link_bss_handler fill "links"; the
	 * print_link_sta fields fill "stats".
	 */
	if (iw_json) {
		json_obj_begin(NULL);
		json_str(JSON_OUT_JSON, "dev", NULL, argv[0]);
		json_arr_begin("links");
	}
	...
	if (!lr.link_found) {
		if (iw_json) {
			json_bool(JSON_OUT_JSON, "connected", NULL, false);
			json_bool(JSON_OUT_JSON, "mld", NULL, false);
			json_obj_end();
		} else if (!lr.anything_found) {
	...
	if (iw_json) {
		json_bool(JSON_OUT_JSON, "connected", NULL, true);
		json_bool(JSON_OUT_JSON, "mld", NULL, lr.mld);
		if (lr.mld)
			json_str(JSON_OUT_JSON, "mld_addr", NULL, addr_buf);
	} else if (lr.mld) {
		printf("MLD %s stats:\n", addr_buf);
	}
	...
	if (iw_json)
		json_obj_begin("stats");
	err = handle_cmd(state, id, 4, station_argv);
	if (iw_json) {
		json_obj_end();	/* stats */
		json_obj_end();	/* wrapper */
	}
	return err;
}
