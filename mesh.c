...
/* Getter */
/* emit one mesh parameter as a JSON field keyed by its name. Units carried in
 * the printf path are dropped — the field name implies them (see schema). */
static void _mesh_param_json(const struct mesh_param_descr *mdescr,
			     struct nlattr *a)
{
	if (mdescr->nla_print_fn == _print_u32_power_mode) {
		const char *mode;

		switch (nla_get_u32(a)) {
		case NL80211_MESH_POWER_ACTIVE: mode = "active"; break;
		case NL80211_MESH_POWER_LIGHT_SLEEP: mode = "light"; break;
		case NL80211_MESH_POWER_DEEP_SLEEP: mode = "deep"; break;
		default: mode = "undefined"; break;
		}
		json_str(JSON_OUT_JSON, mdescr->name, NULL, mode);
		return;
	}
	if (mdescr->nla_print_fn == _print_s32_in_dBm) {
		json_i32(JSON_OUT_JSON, mdescr->name, NULL, (int32_t)nla_get_u32(a));
		return;
	}
	switch (nla_len(a)) {
	case 1: json_u32(JSON_OUT_JSON, mdescr->name, NULL, nla_get_u8(a)); break;
	case 2: json_u32(JSON_OUT_JSON, mdescr->name, NULL, nla_get_u16(a)); break;
	default: json_u32(JSON_OUT_JSON, mdescr->name, NULL, nla_get_u32(a)); break;
	}
}
	...
	if (iw_json) {
		...
		if (mdescr) {
			if (mesh_params[mdescr->mesh_param_num])
				_mesh_param_json(mdescr, mesh_params[mdescr->mesh_param_num]);
		} else {
			for (i = 0; i < ARRAY_SIZE(_mesh_param_descrs); i++) {
