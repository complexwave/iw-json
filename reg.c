...
#define PARSE_FLAG(nl_flag, string_value)  do { \
	...
	if (iw_json) {
		char country[3] = { alpha2[0], alpha2[1], '\0' };

		json_obj_begin(NULL);
		if (tb_msg[NL80211_ATTR_WIPHY]) {
			json_str(JSON_OUT_JSON, "scope", NULL, "phy");
			json_u32(JSON_OUT_JSON, "wiphy", NULL,
				   nla_get_u32(tb_msg[NL80211_ATTR_WIPHY]));
			json_bool(JSON_OUT_JSON, "self_managed", NULL,
				   !!tb_msg[NL80211_ATTR_WIPHY_SELF_MANAGED_REG]);
		} else {
			json_str(JSON_OUT_JSON, "scope", NULL, "global");
		}
		json_str(JSON_OUT_JSON, "country", NULL, country);
		json_str(JSON_OUT_JSON, "dfs_region", NULL,
			     dfs_domain_name(dfs_domain));

		json_arr_begin("rules");
		nla_for_each_nested(nl_rule, tb_msg[NL80211_ATTR_REG_RULES], rem_rule) {
			...
			json_obj_begin(NULL);
			json_u32(JSON_OUT_JSON, "start_mhz", NULL,
				   KHZ_TO_MHZ(nla_get_u32(tb_rule[NL80211_ATTR_FREQ_RANGE_START])));
			json_u32(JSON_OUT_JSON, "end_mhz", NULL,
				   KHZ_TO_MHZ(nla_get_u32(tb_rule[NL80211_ATTR_FREQ_RANGE_END])));
			json_u32(JSON_OUT_JSON, "max_bw_mhz", NULL,
				   KHZ_TO_MHZ(nla_get_u32(tb_rule[NL80211_ATTR_FREQ_RANGE_MAX_BW])));
			if (MBI_TO_DBI(nla_get_u32(tb_rule[NL80211_ATTR_POWER_RULE_MAX_ANT_GAIN])))
				json_i32(JSON_OUT_JSON, "max_ant_gain_dbi", NULL,
					  MBI_TO_DBI(nla_get_u32(tb_rule[NL80211_ATTR_POWER_RULE_MAX_ANT_GAIN])));
			json_i32(JSON_OUT_JSON, "max_eirp_dbm", NULL,
				  MBM_TO_DBM(nla_get_u32(tb_rule[NL80211_ATTR_POWER_RULE_MAX_EIRP])));
			if ((flags & NL80211_RRF_DFS) && tb_rule[NL80211_ATTR_DFS_CAC_TIME])
				json_u32(JSON_OUT_JSON, "dfs_cac_ms", NULL,
					   nla_get_u32(tb_rule[NL80211_ATTR_DFS_CAC_TIME]));

			json_arr_begin("flags");
#define JSON_FLAG(nl_flag, str) do { \
			if (flags & (nl_flag)) \
				json_str(JSON_OUT_JSON, NULL, NULL, str); \
		} while (0)
			JSON_FLAG(NL80211_RRF_NO_OFDM, "NO-OFDM");
			JSON_FLAG(NL80211_RRF_NO_CCK, "NO-CCK");
			JSON_FLAG(NL80211_RRF_NO_INDOOR, "NO-INDOOR");
			JSON_FLAG(NL80211_RRF_NO_OUTDOOR, "NO-OUTDOOR");
			JSON_FLAG(NL80211_RRF_DFS, "DFS");
			JSON_FLAG(NL80211_RRF_PTP_ONLY, "PTP-ONLY");
			JSON_FLAG(NL80211_RRF_AUTO_BW, "AUTO-BW");
			JSON_FLAG(NL80211_RRF_IR_CONCURRENT, "IR-CONCURRENT");
			JSON_FLAG(NL80211_RRF_NO_HT40MINUS, "NO-HT40MINUS");
			JSON_FLAG(NL80211_RRF_NO_HT40PLUS, "NO-HT40PLUS");
			JSON_FLAG(NL80211_RRF_NO_80MHZ, "NO-80MHZ");
			JSON_FLAG(NL80211_RRF_NO_160MHZ, "NO-160MHZ");
			JSON_FLAG(NL80211_RRF_NO_HE, "NO-HE");
			JSON_FLAG(NL80211_RRF_NO_320MHZ, "NO-320MHZ");
			JSON_FLAG(NL80211_RRF_NO_EHT, "NO-EHT");
			if ((flags & NL80211_RRF_NO_IR) && (flags & __NL80211_RRF_NO_IBSS)) {
				json_str(JSON_OUT_JSON, NULL, NULL, "NO-IR");
			} else {
				JSON_FLAG(NL80211_RRF_PASSIVE_SCAN, "PASSIVE-SCAN");
				JSON_FLAG(__NL80211_RRF_NO_IBSS, "NO-IBSS");
			}
#undef JSON_FLAG
			json_arr_end();
