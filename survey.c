...
static int print_survey_handler(struct nl_msg *msg, void *arg)
{
	...
	if (iw_json) {
		...
		if (sinfo[NL80211_SURVEY_INFO_FREQUENCY]) {
			...
			json_bool(JSON_OUT_JSON, "in_use", NULL,
				   !!sinfo[NL80211_SURVEY_INFO_IN_USE]);
		}
		if (sinfo[NL80211_SURVEY_INFO_NOISE])
			json_i32(JSON_OUT_JSON, "noise_dbm", NULL,
				  (int8_t)nla_get_u8(sinfo[NL80211_SURVEY_INFO_NOISE]));
		if (sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME])
			json_u64(JSON_OUT_JSON, "channel_active_time", NULL,
				  nla_get_u64(sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME]));
		if (sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_BUSY])
			json_u64(JSON_OUT_JSON, "channel_busy_time", NULL,
				  nla_get_u64(sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_BUSY]));
		if (sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_EXT_BUSY])
			json_u64(JSON_OUT_JSON, "channel_ext_busy_time", NULL,
				  nla_get_u64(sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_EXT_BUSY]));
		if (sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_RX])
			json_u64(JSON_OUT_JSON, "channel_rx_time", NULL,
				  nla_get_u64(sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_RX]));
		if (sinfo[NL80211_SURVEY_INFO_TIME_BSS_RX])
			json_u64(JSON_OUT_JSON, "channel_bss_rx_time", NULL,
				  nla_get_u64(sinfo[NL80211_SURVEY_INFO_TIME_BSS_RX]));
		if (sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_TX])
			json_u64(JSON_OUT_JSON, "channel_tx_time", NULL,
