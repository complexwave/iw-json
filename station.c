...
static const char *power_mode_str(struct nlattr *a)
{
	switch (nla_get_u32(a)) {
...
{
	printf("%s", power_mode_str(a));
}

static const char *plink_state_str(uint8_t state)
{
	switch (state) {
	case LISTEN:	return "LISTEN";
	case OPN_SNT:	return "OPN_SNT";
	case OPN_RCVD:	return "OPN_RCVD";
	case CNF_RCVD:	return "CNF_RCVD";
...
static void print_bitrate_json(const char *key, struct nlattr *bitrate_attr)
{
	int rate = 0;
	struct nlattr *rinfo[NL80211_RATE_INFO_MAX + 1];
	static struct nla_policy rate_policy[NL80211_RATE_INFO_MAX + 1] = {
	...
	if (rinfo[NL80211_RATE_INFO_BITRATE32])
		...
		rate = nla_get_u16(rinfo[NL80211_RATE_INFO_BITRATE]);
	if (rate > 0)
		json_f64(JSON_OUT_JSON, "bitrate_mbps", NULL, rate / 10.0);

	if (rinfo[NL80211_RATE_INFO_MCS])
		json_u32(JSON_OUT_JSON, "mcs", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_MCS]));
	if (rinfo[NL80211_RATE_INFO_VHT_MCS])
		json_u32(JSON_OUT_JSON, "vht_mcs", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_VHT_MCS]));
	if (rinfo[NL80211_RATE_INFO_S1G_MCS])
		json_u32(JSON_OUT_JSON, "s1g_mcs", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_S1G_MCS]));
	if (rinfo[NL80211_RATE_INFO_HE_MCS])
		json_u32(JSON_OUT_JSON, "he_mcs", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_HE_MCS]));
	if (rinfo[NL80211_RATE_INFO_EHT_MCS])
		json_u32(JSON_OUT_JSON, "eht_mcs", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_EHT_MCS]));

	if (rinfo[NL80211_RATE_INFO_40_MHZ_WIDTH])
		json_u32(JSON_OUT_JSON, "width_mhz", NULL, 40);
	else if (rinfo[NL80211_RATE_INFO_80_MHZ_WIDTH])
		json_u32(JSON_OUT_JSON, "width_mhz", NULL, 80);
	else if (rinfo[NL80211_RATE_INFO_80P80_MHZ_WIDTH]) {
		json_u32(JSON_OUT_JSON, "width_mhz", NULL, 80);
		json_bool(JSON_OUT_JSON, "width_80p80", NULL, true);
	} else if (rinfo[NL80211_RATE_INFO_160_MHZ_WIDTH])
		json_u32(JSON_OUT_JSON, "width_mhz", NULL, 160);
	else if (rinfo[NL80211_RATE_INFO_320_MHZ_WIDTH])
		json_u32(JSON_OUT_JSON, "width_mhz", NULL, 320);
	else if (rinfo[NL80211_RATE_INFO_1_MHZ_WIDTH])
		json_u32(JSON_OUT_JSON, "width_mhz", NULL, 1);
	else if (rinfo[NL80211_RATE_INFO_2_MHZ_WIDTH])
		json_u32(JSON_OUT_JSON, "width_mhz", NULL, 2);
	else if (rinfo[NL80211_RATE_INFO_4_MHZ_WIDTH])
		json_u32(JSON_OUT_JSON, "width_mhz", NULL, 4);
	else if (rinfo[NL80211_RATE_INFO_8_MHZ_WIDTH])
		json_u32(JSON_OUT_JSON, "width_mhz", NULL, 8);
	else if (rinfo[NL80211_RATE_INFO_16_MHZ_WIDTH])
		json_u32(JSON_OUT_JSON, "width_mhz", NULL, 16);

	if (rinfo[NL80211_RATE_INFO_SHORT_GI])
		json_bool(JSON_OUT_JSON, "short_gi", NULL, true);

	if (rinfo[NL80211_RATE_INFO_VHT_NSS])
		json_u32(JSON_OUT_JSON, "nss", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_VHT_NSS]));
	else if (rinfo[NL80211_RATE_INFO_S1G_NSS])
		json_u32(JSON_OUT_JSON, "nss", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_S1G_NSS]));
	else if (rinfo[NL80211_RATE_INFO_HE_NSS])
		json_u32(JSON_OUT_JSON, "nss", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_HE_NSS]));
	else if (rinfo[NL80211_RATE_INFO_EHT_NSS])
		json_u32(JSON_OUT_JSON, "nss", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_EHT_NSS]));

	if (rinfo[NL80211_RATE_INFO_HE_GI])
		json_u32(JSON_OUT_JSON, "he_gi", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_HE_GI]));
	if (rinfo[NL80211_RATE_INFO_HE_DCM])
		json_u32(JSON_OUT_JSON, "he_dcm", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_HE_DCM]));
	if (rinfo[NL80211_RATE_INFO_HE_RU_ALLOC])
		json_u32(JSON_OUT_JSON, "he_ru_alloc", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_HE_RU_ALLOC]));
	if (rinfo[NL80211_RATE_INFO_EHT_GI])
		json_u32(JSON_OUT_JSON, "eht_gi", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_EHT_GI]));
	if (rinfo[NL80211_RATE_INFO_EHT_RU_ALLOC])
		json_u32(JSON_OUT_JSON, "eht_ru_alloc", NULL,
			   nla_get_u8(rinfo[NL80211_RATE_INFO_EHT_RU_ALLOC]));

	json_obj_end();
}

static void print_chain_signal_json(const char *key, struct nlattr *attr_list)
{
	struct nlattr *attr;
	int rem;
	...
	json_arr_begin(key);
	nla_for_each_nested(attr, attr_list, rem)
		json_i32(JSON_OUT_JSON, NULL, NULL, (int8_t)nla_get_u8(attr));
	json_arr_end();
}

static void print_txq_stats_json(struct nlattr *txqstats_attr)
{
	struct nlattr *ti[NL80211_TXQ_STATS_MAX + 1];
	static struct nla_policy txqstats_policy[NL80211_TXQ_STATS_MAX + 1] = {
		[NL80211_TXQ_STATS_BACKLOG_BYTES] = { .type = NLA_U32 },
		[NL80211_TXQ_STATS_BACKLOG_PACKETS] = { .type = NLA_U32 },
		[NL80211_TXQ_STATS_FLOWS] = { .type = NLA_U32 },
	...
	if (nla_parse_nested(ti, NL80211_TXQ_STATS_MAX, txqstats_attr,
			     txqstats_policy))
		return;

	json_obj_begin("txq_stats");
#define P(attr, name) \
	if (ti[NL80211_TXQ_STATS_ ## attr]) \
		json_u32(JSON_OUT_JSON, name, NULL, \
			   nla_get_u32(ti[NL80211_TXQ_STATS_ ## attr]))
	P(BACKLOG_BYTES, "backlog_bytes");
	P(BACKLOG_PACKETS, "backlog_packets");
	P(FLOWS, "flows");
	P(DROPS, "drops");
	P(ECN_MARKS, "ecn_marks");
	P(OVERLIMIT, "overlimit");
	P(COLLISIONS, "collisions");
	P(TX_BYTES, "tx_bytes");
	P(TX_PACKETS, "tx_packets");
#undef P
	json_obj_end();
}

static void print_tid_stats_json(struct nlattr *tid_stats_attr)
{
	struct nlattr *si[NL80211_TID_STATS_MAX + 1], *tidattr, *info;
	static struct nla_policy stats_policy[NL80211_TID_STATS_MAX + 1] = {
		[NL80211_TID_STATS_RX_MSDU] = { .type = NLA_U64 },
		[NL80211_TID_STATS_TX_MSDU] = { .type = NLA_U64 },
		[NL80211_TID_STATS_TX_MSDU_RETRIES] = { .type = NLA_U64 },
	...
	json_arr_begin("tid_stats");
	nla_for_each_nested(tidattr, tid_stats_attr, rem) {
		if (nla_parse_nested(si, NL80211_TID_STATS_MAX, tidattr,
				     stats_policy))
			continue;
		json_obj_begin(NULL);
		json_u32(JSON_OUT_JSON, "tid", NULL, i++);
		info = si[NL80211_TID_STATS_RX_MSDU];
		if (info)
			json_u64(JSON_OUT_JSON, "rx_msdu", NULL, nla_get_u64(info));
		info = si[NL80211_TID_STATS_TX_MSDU];
		if (info)
			json_u64(JSON_OUT_JSON, "tx_msdu", NULL, nla_get_u64(info));
		info = si[NL80211_TID_STATS_TX_MSDU_RETRIES];
		if (info)
			json_u64(JSON_OUT_JSON, "tx_msdu_retries", NULL,
				  nla_get_u64(info));
		info = si[NL80211_TID_STATS_TX_MSDU_FAILED];
		if (info)
			json_u64(JSON_OUT_JSON, "tx_msdu_failed", NULL,
				  nla_get_u64(info));
		info = si[NL80211_TID_STATS_TXQ_STATS];
		if (info)
			print_txq_stats_json(info);
		json_obj_end();
	}
	json_arr_end();
}

static void print_bss_param_json(struct nlattr *bss_param_attr)
{
	struct nlattr *bi[NL80211_STA_BSS_PARAM_MAX + 1], *info;
	...
	if (nla_parse_nested(bi, NL80211_STA_BSS_PARAM_MAX, bss_param_attr,
			     bss_policy))
		return;

	json_obj_begin("bss_param");
	info = bi[NL80211_STA_BSS_PARAM_DTIM_PERIOD];
	if (info)
		json_u32(JSON_OUT_JSON, "dtim_period", NULL, nla_get_u8(info));
	info = bi[NL80211_STA_BSS_PARAM_BEACON_INTERVAL];
	if (info)
		json_u32(JSON_OUT_JSON, "beacon_interval", NULL,
			   nla_get_u16(info));
	json_bool(JSON_OUT_JSON, "cts_prot", NULL,
		   !!bi[NL80211_STA_BSS_PARAM_CTS_PROT]);
	json_bool(JSON_OUT_JSON, "short_preamble", NULL,
		   !!bi[NL80211_STA_BSS_PARAM_SHORT_PREAMBLE]);
	json_bool(JSON_OUT_JSON, "short_slot_time", NULL,
		   !!bi[NL80211_STA_BSS_PARAM_SHORT_SLOT_TIME]);
	json_obj_end();
}

void print_nested_sta_json(struct nlattr *si[NL80211_STA_INFO_MAX + 1])
{
	if (si[NL80211_STA_INFO_INACTIVE_TIME])
		json_u32(JSON_OUT_JSON, "inactive_time", NULL,
			   nla_get_u32(si[NL80211_STA_INFO_INACTIVE_TIME]));
	if (si[NL80211_STA_INFO_RX_BYTES64])
		json_u64(JSON_OUT_JSON, "rx_bytes", NULL,
			  nla_get_u64(si[NL80211_STA_INFO_RX_BYTES64]));
	else if (si[NL80211_STA_INFO_RX_BYTES])
		json_u32(JSON_OUT_JSON, "rx_bytes", NULL,
			   nla_get_u32(si[NL80211_STA_INFO_RX_BYTES]));
	if (si[NL80211_STA_INFO_RX_PACKETS])
		json_u32(JSON_OUT_JSON, "rx_packets", NULL,
			   nla_get_u32(si[NL80211_STA_INFO_RX_PACKETS]));
	if (si[NL80211_STA_INFO_TX_BYTES64])
		json_u64(JSON_OUT_JSON, "tx_bytes", NULL,
			  nla_get_u64(si[NL80211_STA_INFO_TX_BYTES64]));
	else if (si[NL80211_STA_INFO_TX_BYTES])
		json_u32(JSON_OUT_JSON, "tx_bytes", NULL,
			   nla_get_u32(si[NL80211_STA_INFO_TX_BYTES]));
	if (si[NL80211_STA_INFO_TX_PACKETS])
		json_u32(JSON_OUT_JSON, "tx_packets", NULL,
			   nla_get_u32(si[NL80211_STA_INFO_TX_PACKETS]));
	if (si[NL80211_STA_INFO_TX_RETRIES])
		json_u32(JSON_OUT_JSON, "tx_retries", NULL,
			   nla_get_u32(si[NL80211_STA_INFO_TX_RETRIES]));
	if (si[NL80211_STA_INFO_TX_FAILED])
		json_u32(JSON_OUT_JSON, "tx_failed", NULL,
			   nla_get_u32(si[NL80211_STA_INFO_TX_FAILED]));
	if (si[NL80211_STA_INFO_BEACON_LOSS])
		json_u32(JSON_OUT_JSON, "beacon_loss", NULL,
			   nla_get_u32(si[NL80211_STA_INFO_BEACON_LOSS]));
	if (si[NL80211_STA_INFO_BEACON_RX])
		json_u64(JSON_OUT_JSON, "beacon_rx", NULL,
			  nla_get_u64(si[NL80211_STA_INFO_BEACON_RX]));
	if (si[NL80211_STA_INFO_RX_DROP_MISC])
		json_u64(JSON_OUT_JSON, "rx_drop_misc", NULL,
			  nla_get_u64(si[NL80211_STA_INFO_RX_DROP_MISC]));
	if (si[NL80211_STA_INFO_SIGNAL])
		json_i32(JSON_OUT_JSON, "signal_dbm", NULL,
			  (int8_t)nla_get_u8(si[NL80211_STA_INFO_SIGNAL]));
	print_chain_signal_json("signal_chains",
				si[NL80211_STA_INFO_CHAIN_SIGNAL]);
	if (si[NL80211_STA_INFO_SIGNAL_AVG])
		json_i32(JSON_OUT_JSON, "signal_avg_dbm", NULL,
			  (int8_t)nla_get_u8(si[NL80211_STA_INFO_SIGNAL_AVG]));
	print_chain_signal_json("signal_avg_chains",
				si[NL80211_STA_INFO_CHAIN_SIGNAL_AVG]);
	if (si[NL80211_STA_INFO_BEACON_SIGNAL_AVG])
		json_i32(JSON_OUT_JSON, "beacon_signal_avg_dbm", NULL,
			  (int8_t)nla_get_u8(si[NL80211_STA_INFO_BEACON_SIGNAL_AVG]));
	if (si[NL80211_STA_INFO_T_OFFSET])
		json_i64(JSON_OUT_JSON, "t_offset_us", NULL,
			  (int64_t)nla_get_s64(si[NL80211_STA_INFO_T_OFFSET]));
	if (si[NL80211_STA_INFO_TX_BITRATE])
		print_bitrate_json("tx_bitrate", si[NL80211_STA_INFO_TX_BITRATE]);
	if (si[NL80211_STA_INFO_TX_DURATION])
		json_u64(JSON_OUT_JSON, "tx_duration_us", NULL,
			  nla_get_u64(si[NL80211_STA_INFO_TX_DURATION]));
	if (si[NL80211_STA_INFO_RX_BITRATE])
		print_bitrate_json("rx_bitrate", si[NL80211_STA_INFO_RX_BITRATE]);
	if (si[NL80211_STA_INFO_RX_DURATION])
		json_u64(JSON_OUT_JSON, "rx_duration_us", NULL,
			  nla_get_u64(si[NL80211_STA_INFO_RX_DURATION]));
	if (si[NL80211_STA_INFO_ACK_SIGNAL])
		json_i32(JSON_OUT_JSON, "ack_signal_dbm", NULL,
			  (int8_t)nla_get_u8(si[NL80211_STA_INFO_ACK_SIGNAL]));
	if (si[NL80211_STA_INFO_ACK_SIGNAL_AVG])
		json_i32(JSON_OUT_JSON, "ack_signal_avg_dbm", NULL,
			  (int8_t)nla_get_u8(si[NL80211_STA_INFO_ACK_SIGNAL_AVG]));
	if (si[NL80211_STA_INFO_AIRTIME_WEIGHT])
		json_u32(JSON_OUT_JSON, "airtime_weight", NULL,
			   nla_get_u16(si[NL80211_STA_INFO_AIRTIME_WEIGHT]));
	if (si[NL80211_STA_INFO_EXPECTED_THROUGHPUT]) {
		uint32_t thr = nla_get_u32(si[NL80211_STA_INFO_EXPECTED_THROUGHPUT]);

		json_f64(JSON_OUT_JSON, "expected_throughput_mbps", NULL,
			    thr * 1000 / 1024 / 1000.0);
	}
	if (si[NL80211_STA_INFO_TID_STATS])
		print_tid_stats_json(si[NL80211_STA_INFO_TID_STATS]);
	if (si[NL80211_STA_INFO_BSS_PARAM])
		print_bss_param_json(si[NL80211_STA_INFO_BSS_PARAM]);
	if (si[NL80211_STA_INFO_CONNECTED_TIME])
		json_u32(JSON_OUT_JSON, "connected_time", NULL,
			   nla_get_u32(si[NL80211_STA_INFO_CONNECTED_TIME]));
	if (si[NL80211_STA_INFO_ASSOC_AT_BOOTTIME]) {
		struct timespec now_ts;
		struct timeval now;
		unsigned long long boot_ns, bt, now_ms;

		gettimeofday(&now, NULL);
		now_ms = now.tv_sec * 1000ULL + now.tv_usec / 1000;
		clock_gettime(CLOCK_BOOTTIME, &now_ts);
		boot_ns = now_ts.tv_sec * 1000000000ULL + now_ts.tv_nsec;
		bt = nla_get_u64(si[NL80211_STA_INFO_ASSOC_AT_BOOTTIME]);
		json_u64(JSON_OUT_JSON, "assoc_at_ms", NULL,
			  now_ms - ((boot_ns - bt) / 1000000));
	}
}
...
static int print_sta_handler(struct nl_msg *msg, void *arg)
{
	...
	if (iw_json) {
		json_obj_begin(NULL);
		json_str(JSON_OUT_JSON, "addr", NULL, mac_addr);
		json_str(JSON_OUT_JSON, "dev", NULL, dev);

		if (sinfo[NL80211_STA_INFO_LLID])
			json_u32(JSON_OUT_JSON, "mesh_llid", NULL,
				   nla_get_u16(sinfo[NL80211_STA_INFO_LLID]));
		if (sinfo[NL80211_STA_INFO_PLID])
			json_u32(JSON_OUT_JSON, "mesh_plid", NULL,
				   nla_get_u16(sinfo[NL80211_STA_INFO_PLID]));
		if (sinfo[NL80211_STA_INFO_PLINK_STATE])
			json_str(JSON_OUT_JSON, "mesh_plink", NULL,
				     plink_state_str(nla_get_u8(sinfo[NL80211_STA_INFO_PLINK_STATE])));
		if (sinfo[NL80211_STA_INFO_AIRTIME_LINK_METRIC])
			json_u32(JSON_OUT_JSON, "mesh_airtime_link_metric", NULL,
				   nla_get_u32(sinfo[NL80211_STA_INFO_AIRTIME_LINK_METRIC]));
		if (sinfo[NL80211_STA_INFO_CONNECTED_TO_GATE])
			json_bool(JSON_OUT_JSON, "mesh_connected_to_gate", NULL,
				   !!nla_get_u8(sinfo[NL80211_STA_INFO_CONNECTED_TO_GATE]));
		if (sinfo[NL80211_STA_INFO_CONNECTED_TO_AS])
			json_bool(JSON_OUT_JSON, "mesh_connected_to_as", NULL,
				   !!nla_get_u8(sinfo[NL80211_STA_INFO_CONNECTED_TO_AS]));
		if (sinfo[NL80211_STA_INFO_LOCAL_PM])
			json_str(JSON_OUT_JSON, "mesh_local_ps_mode", NULL,
				     power_mode_str(sinfo[NL80211_STA_INFO_LOCAL_PM]));
		if (sinfo[NL80211_STA_INFO_PEER_PM])
			json_str(JSON_OUT_JSON, "mesh_peer_ps_mode", NULL,
				     power_mode_str(sinfo[NL80211_STA_INFO_PEER_PM]));
		if (sinfo[NL80211_STA_INFO_NONPEER_PM])
			json_str(JSON_OUT_JSON, "mesh_nonpeer_ps_mode", NULL,
				     power_mode_str(sinfo[NL80211_STA_INFO_NONPEER_PM]));

		if (sinfo[NL80211_STA_INFO_STA_FLAGS]) {
			sta_flags = (struct nl80211_sta_flag_update *)
				    nla_data(sinfo[NL80211_STA_INFO_STA_FLAGS]);
#define FLAG(bit, name) \
			if (sta_flags->mask & BIT(NL80211_STA_FLAG_ ## bit)) \
				json_bool(JSON_OUT_JSON, name, NULL, \
					   !!(sta_flags->set & BIT(NL80211_STA_FLAG_ ## bit)))
			FLAG(AUTHORIZED, "authorized");
			FLAG(AUTHENTICATED, "authenticated");
			FLAG(ASSOCIATED, "associated");
			FLAG(SHORT_PREAMBLE, "short_preamble");
			FLAG(WME, "wmm");
			FLAG(MFP, "mfp");
			FLAG(TDLS_PEER, "tdls_peer");
#undef FLAG
		}

		print_nested_sta_json(sinfo);

		if (tb[NL80211_ATTR_MLO_LINKS]) {
			int ret = 0;

			json_arr_begin("mlo_links");
			nla_for_each_nested(attrs, tb[NL80211_ATTR_MLO_LINKS], ret) {
				...
				if (!link[NL80211_ATTR_MLO_LINK_ID])
					continue;
				json_obj_begin(NULL);
				json_u32(JSON_OUT_JSON, "link_id", NULL,
				...
				if (link[NL80211_ATTR_STA_INFO] &&
				    !nla_parse_nested(link_sinfo, NL80211_STA_INFO_MAX,
						      link[NL80211_ATTR_STA_INFO], stats_policy))
					print_nested_sta_json(link_sinfo);
				json_obj_end();
			}
			json_arr_end();
		}
